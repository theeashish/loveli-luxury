...
test('Partner Agreement preserves the established programme clauses without visitor-facing internal review language', async ({ page }) => {
  ...
  const contents = page.getByRole('navigation', { name: 'In this agreement' })
  await expect(contents).toBeVisible()
  await expect(contents.locator('a[href="#4-sales-and-commission"]')).toBeVisible()
  await expect(contents.locator('a[href="#10-suspension-and-termination"]')).toBeVisible()

  await expect(page.getByRole('heading', { level: 2, name: '4. Sales and commission' })).toBeVisible()
...
test('Partner Agreement remains readable and navigable on a phone viewport', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1, name: 'Partner Agreement' })).toBeVisible()
  await expect(page.getByRole('navigation', { name: 'In this agreement' })).toBeVisible()
  await expect(page.locator('a[href="#5-prices-and-customers"]')).toBeVisible()
  await expect(page.getByText('Draft for legal review only', { exact: false })).toHaveCount(0)
})
