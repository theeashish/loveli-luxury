...
const PUBLIC_VARIANT_COLUMNS =
  "id, product_id, sku, size_ml, retail_price_minor, distributor_price_minor, selling_price_minor, pv_per_bottle, weight_g, inventory_qty, is_active, created_at";
...
export async function getProductBySlug(
  ...
  const fragranceMeta = (
    metaRes.error ? null : metaRes.data
  ) as FragranceMetaRow | null;
  return mapProduct(
    data,
    variantsRes.data ?? [],
    imagesRes.data ?? [],
...
async function hydrateBundle(
  ...
  const hydrated = items.map((it) => {
    ...
    if (!v)
      throw new Error(
        `bundle_items references missing variant ${it.variant_id}`,
      );
    const p = productById.get(v.product_id);
    if (!p)
      throw new Error(`product ${v.product_id} missing for variant ${v.id}`);
    return {
      variant_id: v.id,
