 ...
 *   6. record_internal_pv_allocations (non-fatal; restricted ledger).
 *   7. sendOrderReceipt (no-op without RESEND env; non-fatal).
 *   8. audit_log entry tagged with the path that drove the reconcile.
...
export async function applyPaymentSuccess(
    ...
    if (orderRes.error) {
      ...
      if (activationMode) {
        ...
        if (distRes.error) {
          ...
          if (roleRes.error)
            warnings.push(`activation role: ${roleRes.error.message}`);
        }
      } else {
  ...
  if (commissionFailed) {
    ...
    if (commissionAuditRes.error) {
      warnings.push(
        "commission failure audit: " + commissionAuditRes.error.message,
      );
    }
  }

  // 6. Internal marketing/management PV — non-fatal. The RPC is idempotent,
  // requires a paid order, and writes only the restricted internal ledger.
  const internalPvRes = (await service.rpc("record_internal_pv_allocations", {
    p_order_id: input.orderId,
  })) as { error: { message: string } | null };
  if (internalPvRes.error) {
    warnings.push(
      "record_internal_pv_allocations: " + internalPvRes.error.message,
    );
    const internalPvAuditRes = await service.from("audit_log").insert({
      actor_id: input.actorId ?? null,
      action: "internal_pv.allocations_write_failed",
      resource_type: "order",
      resource_id: String(input.orderId),
      after_data: {
        order_id: input.orderId,
        invoice_id: input.invoiceId,
        source: input.source,
        error: internalPvRes.error.message,
      },
    });
    if (internalPvAuditRes.error) {
      warnings.push(
        "internal PV failure audit: " + internalPvAuditRes.error.message,
      );
    }
  }

  // 7. Receipt email — non-fatal; no-op without RESEND env.
  try {
  ...
  // 8. Audit row tagged with the path that drove the reconcile.
