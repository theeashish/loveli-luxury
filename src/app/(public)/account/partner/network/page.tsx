/**
 * Network — every partner sponsored beneath the current partner, up to the
 * 7-level cap, rendered as an indented tree (sponsor → invitees) using the
 * direct-sponsor edge in `distributors.sponsor_id`. Per-level counters at
 * the top give an at-a-glance shape.
 *
 * Each member's standing is shown via the Phase-1 rank→tier display bridge
 * (`partnerTierForRank`) so the network reads in customer-facing tier
 * language, not internal rank names. Email + phone stay hidden — admins see
 * those, partners don't.
 *
 * Renamed from /account/partner/downline in the Phase 1 slug cleanup; a 301
 * redirect from the old path lives in next.config.js.
 */
...
export default async function NetworkPage() {
  ...
  // Build the tree. Root nodes are partners whose sponsor_id === me.id.
    ...
    <li>
      ...
      >
        ...
        <span className="text-xs text-[hsl(var(--muted-foreground))]">
          {partnerTierForRank(node.rank?.rank_position ?? null).displayName}
        </span>
