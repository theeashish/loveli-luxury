...
async function getLiveEarningsData(): Promise<{
  ...
  const [productsRes, variantsRes, ratesRes] = await Promise.all([
    service
      .from('products')
      .select('id, name')
      .eq('is_active', true)
    ...
    service
      ...
      .select(
        'id, product_id, size_ml, distributor_price_minor, retail_price_minor, pv_per_bottle',
      )
      .eq('is_active', true)
  ...
  const productById = new Map((productsRes.data ?? []).map((product) => [product.id, product.name]))
  const pricing = (variantsRes.data ?? []).map((variant) => {
    ...
    return {
      id: variant.id,
      productName: productById.get(variant.product_id) ?? `Product ${variant.product_id}`,
...
export default async function PartnerEarningsPage() {
  const { pricing, rates } = await getLiveEarningsData()

  return (
    <div className="space-y-12">
      <div className="flex items-center">
        <Image
          src="/loveli-luxury-logo.png"
          alt="Loveli Luxury Scents"
          width={112}
          height={120}
      ...
      <div className="rounded-md border border-[hsl(var(--primary))]/30 bg-[hsl(var(--primary))]/5 px-4 py-3 text-xs text-[hsl(var(--muted-foreground))]">
        These figures are confidential to Loveli Luxury partners. Please keep them within the partner community.
      </div>

      <section>
        <h2 className="font-serif text-2xl">Product pricing &amp; your retail margin</h2>
        <p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">
          Prices, PV, and margins below are read from the current active catalog. They update when the catalog is changed.
        </p>
              ...
              >
                ...
                <dl className="mt-5 space-y-2 text-sm">
                  <div className="flex justify-between border-b border-[hsl(var(--border))]/60 pb-2">
                    <dt className="text-[hsl(var(--muted-foreground))]">Your purchase price</dt>
                    <dd className="font-medium">{formatKes(product.purchaseMinor)}</dd>
                  </div>
                  <div className="flex justify-between border-b border-[hsl(var(--border))]/60 pb-2">
                    <dt className="text-[hsl(var(--muted-foreground))]">Mandatory retail price</dt>
                    <dd className="font-medium text-[hsl(var(--brand-gold))]">
                      {formatKes(product.retailMinor)}
                    </dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-[hsl(var(--muted-foreground))]">PV per bottle</dt>
      ...
      <section>
        ...
        <p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">
          Commission rates are read from the active database configuration and applied to PV by the compensation engine.
        </p>
        <div className="mt-6 overflow-hidden rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--muted))]/40">
          {rates.length === 0 ? (
            ...
            <ul className="divide-y divide-[hsl(var(--border))]/60 text-sm">
              {rates.map((rate) => (
                <li key={rate.level} className="flex items-center justify-between px-5 py-3">
                  <span className="text-[hsl(var(--muted-foreground))]">Level {rate.level}</span>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
          {ALL_PARTNER_TIERS.map((tier) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">{tier.tagline}</p>
      ...
      <ul className="mt-2 divide-y divide-[hsl(var(--border))]/60 text-sm">
        {rows.map((row) => (
          <li key={row.qty} className="flex items-center justify-between py-2">
            <span className="text-[hsl(var(--muted-foreground))]">Sell {row.qty} bottles</span>
            <span className="font-medium text-[hsl(var(--brand-gold))]">
