 ...
 *       - IntaSend secret + wallet + webhook challenge are configured
 *         (does NOT hit IntaSend — that would burn quota on the monitor;
 ...
 *     either the DB or the provider on every probe.
 *
 * Response shape (deep mode):
 *   { ok: true|false, status: "ok"|"degraded"|"down",
 *     checks: { db: {ok,ms}, env: {ok}, intasend: {ok}, upstash: {ok}, … },
 *     ts: ISO8601 }
 *
...
function checkIntasendConfigured(): CheckResult {
  try {
    const env = getServerEnv()
    if (!env.INTASEND_PUBLISHABLE_KEY)
      return { ok: false, reason: 'INTASEND_PUBLISHABLE_KEY unset' }
    if (!env.INTASEND_SECRET_TOKEN)
      return { ok: false, reason: 'INTASEND_SECRET_TOKEN unset' }
    if (!env.INTASEND_WALLET_ID)
      return { ok: false, reason: 'INTASEND_WALLET_ID unset' }
    if (!env.INTASEND_WEBHOOK_CHALLENGE)
      return { ok: false, reason: 'INTASEND_WEBHOOK_CHALLENGE unset' }
    // We deliberately do NOT call IntaSend here — that would consume API quota
...
export async function GET(req: NextRequest) {
  ...
  const [db, env, intasend, upstash] = await Promise.all([
    checkDb(),
    Promise.resolve(checkServerEnv()),
    Promise.resolve(checkIntasendConfigured()),
    Promise.resolve(checkUpstashConfigured()),
  ])

  // Critical checks (failure = 503). DB and env are existential — the app
  // cannot serve any meaningful request without them. IntaSend presence is
  ...
  const critical = [db.ok, env.ok, intasend.ok]
  ...
  return NextResponse.json(
    {
      ...
      status,
      mode: 'deep',
      checks: { db, env, intasend, upstash },
