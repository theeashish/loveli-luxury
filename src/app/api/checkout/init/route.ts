...
const phoneSchema = z
  ...
  .regex(
    /^\+254[17]\d{8}$/,
    "Phone must be a Kenyan number like +254712345678 (12 digits)",
  );
...
export function hasBundleCheckoutLine<T extends object>(
  lines: ReadonlyArray<T>,
): boolean {
  return lines.some(
    (line) => (line as { bundleId?: unknown }).bundleId != null,
  );
}
...
export function bundleCheckoutGoneResponse(): NextResponse {
  return NextResponse.json(
    {
      error:
        "Bundle checkout is no longer available. Please shop individual fragrances.",
    },
    { status: 410 },
  );
}
...
export async function POST(req: Request) {
  ...
  const rl = await checkRateLimit("checkout-init", clientIp(req), {
    limit: 5,
    windowSeconds: 60,
  });
  if (!rl.ok) {
  ...
  const roleRes = await service
    ...
    .is("revoked_at", null)
    .maybeSingle();
  const wholesaleRoleRes = await service
    .from("user_roles")
    .select("role")
    .eq("user_id", user.id)
    .eq("role" as never, "wholesale" as never)
    .is("revoked_at", null)
    .maybeSingle();
  const isApprovedWholesaler = Boolean(
    (wholesaleRoleRes.data?.role as string) === "wholesale",
  );
  const distributorRes = await service
    .from("distributors")
    .select("is_active, starter_paid_at")
    .eq("user_id", user.id)
    .maybeSingle();
  const isActiveDistributor = Boolean(
    roleRes.data?.role === "distributor" &&
    distributorRes.data?.is_active &&
    distributorRes.data?.starter_paid_at,
  );
  ...
  const variantsRes = variantIds.length
    ? await service
        ...
        .select(
          "id, retail_price_minor, distributor_price_minor, pv_per_bottle, internal_pv_per_bottle, is_active",
        )
        .in("id", variantIds)
    : { data: [], error: null };
  ...
  if (bundleItemsRes.error) {
    return NextResponse.json(
      { error: "Bundle items lookup failed" },
      { status: 500 },
    );
  }
  ...
  const componentVariantsRes = componentVariantIds.length
    ? await service
        .from("product_variants")
        .select("id, pv_per_bottle, internal_pv_per_bottle")
        .in("id", componentVariantIds)
    : { data: [], error: null };
  if (componentVariantsRes.error) {
    return NextResponse.json(
      { error: "Bundle component PV lookup failed" },
      { status: 500 },
    );
  }
  ...
  const internalPvByVariant = new Map<number, number>(
    componentVariants.map((v) => [v.id, v.internal_pv_per_bottle ?? 0]),
  );
  ...
  for (const line of lines) {
    if (line.kind === "variant") {
      ...
      const expected = isApprovedWholesaler
        ? (BigInt(v.retail_price_minor) * 75n) / 100n
        : isActiveDistributor
          ? BigInt(v.distributor_price_minor)
          : BigInt(v.retail_price_minor);
      ...
      requiredVariantQty.set(
        v.id,
        (requiredVariantQty.get(v.id) ?? 0) + line.qty,
      );
      const lineTotal = expected * BigInt(line.qty);
      subtotalMinor += lineTotal;
      const variantPv = (v as { pv_per_bottle?: number }).pv_per_bottle ?? 0;
      const variantInternalPv =
        (v as { internal_pv_per_bottle?: number }).internal_pv_per_bottle ?? 0;
      itemRows.push({
        variant_id: v.id,
        bundle_id: null,
        quantity: line.qty,
        ...
        commissionable_amount_minor: Number(
          BigInt(v.distributor_price_minor) * BigInt(line.qty),
        ),
        commission_pv: variantPv * line.qty,
        internal_pv: variantInternalPv * line.qty,
      });
    } else {
      ...
      const expected = isApprovedWholesaler
        ? (BigInt(b.retail_price_minor) * 75n) / 100n
        : isActiveDistributor
          ? BigInt(b.distributor_price_minor)
          : BigInt(b.retail_price_minor);
      const claimed = BigInt(line.unitPriceMinor);
      if (expected !== claimed) {
        return NextResponse.json(
          { error: "Prices have changed. Please review your cart." },
      ...
      for (const c of components) {
        ...
        bundleInternalPvPerOne +=
          (internalPvByVariant.get(c.variant_id) ?? 0) * c.quantity;
      }
      ...
      itemRows.push({
        ...
        internal_pv: bundleInternalPvPerOne * line.qty,
      });
    }
  }

  if (isApprovedWholesaler) {
    const bottleCount = Array.from(requiredVariantQty.values()).reduce(
      (sum, qty) => sum + qty,
      0,
    );
    if (bottleCount < 12) {
      return NextResponse.json(
        { error: "Wholesale orders require a minimum of 12 bottles." },
        { status: 400 },
      );
    }
  }
