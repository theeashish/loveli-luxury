    ...
    <section className="border-t border-[hsl(var(--border))]/70 bg-[hsl(var(--muted))]/20">
      <div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
        ...
        <ol className="mt-10 grid gap-3 md:grid-cols-5">
          {plan.ranks.map((rank) => (
            <li key={rank.position} className={`rounded-xl border p-5 ${prestigeClass[rank.prestige]}`}>
              <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-[hsl(var(--primary))]">Rank {rank.position}</p>
              <h3 className="mt-3 font-serif text-2xl text-[hsl(var(--foreground))]">{rank.name}</h3>
              <p className="mt-3 text-sm leading-6 text-[hsl(var(--muted-foreground))]">{rank.commissionLevels}</p>
            </li>
          ))}
        </ol>
        ...
        <div className="mt-16 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-7 md:p-9">
            ...
            <p className="mt-6 text-xs leading-5 text-[hsl(var(--muted-foreground))]">Actual earnings depend on product sales, customer demand, team performance and qualification requirements.</p>
          </div>
        </div>
        ...
        <p className="mt-6 text-xs leading-5 text-[hsl(var(--muted-foreground))]">All rewards are subject to qualification requirements and company terms and conditions.</p>
        ...
        <div className="mt-16 border-y border-[hsl(var(--primary))]/40 py-10 text-center">
          <p className="font-serif text-2xl uppercase tracking-[0.12em] text-[hsl(var(--foreground))] md:text-4xl">Start as a partner. Grow as an entrepreneur. Lead as a director. Build as a Crown President.</p>
          <p className="mt-5 text-sm font-semibold uppercase tracking-[0.24em] text-[hsl(var(--primary))]">Create your Loveli legacy.</p>
          <p className="mx-auto mt-5 max-w-3xl text-xs leading-5 text-[hsl(var(--muted-foreground))]">{plan.incomeDisclaimer}</p>
        </div>
      </div>
    </section>
  )
