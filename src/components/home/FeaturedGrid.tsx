...
function Card({ f, index }: { f: FragranceMeta; index: number }) {
  return (
    <Link
      href={'/p/' + f.slug}
      className={
        'loveli-reveal-up group relative flex min-h-0 flex-col overflow-hidden border border-[hsl(var(--primary)/0.24)] bg-[hsl(var(--brand-charcoal))] p-4 text-[hsl(var(--brand-silver))] transition duration-300 hover:-translate-y-1 hover:border-[hsl(var(--primary)/0.62)] sm:p-7 md:block md:min-h-[28rem] ' +
    ...
    >
      ...
      <div className="relative z-10 mt-4 flex min-h-[8rem] flex-1 flex-col justify-between md:mt-0 md:min-h-[25rem]">
        ...
        <div className="mt-auto max-w-[17rem] border-l border-[hsl(var(--primary)/0.55)] pl-4">
          <h3 className="font-serif text-3xl leading-none tracking-tight text-[hsl(var(--brand-white))] sm:text-4xl">{f.name}</h3>
          <p className="mt-2 text-sm italic text-[hsl(var(--brand-silver))]">{f.tagline}</p>
...
export function FeaturedGrid() {
  ...
  return (
    <section className="relative border-b border-[hsl(var(--primary)/0.24)] bg-[hsl(var(--brand-white))] py-7 text-[hsl(var(--brand-charcoal))] sm:py-10">
      <div className="mx-auto max-w-7xl px-6">
        <header className="loveli-reveal-up mb-7 flex flex-wrap items-end justify-between gap-5 sm:mb-10">
          <div>
            ...
            <h2 className="mt-3 max-w-2xl font-serif text-4xl leading-[0.94] tracking-tight text-[hsl(var(--brand-charcoal))] sm:text-5xl">
