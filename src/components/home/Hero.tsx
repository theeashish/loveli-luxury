...
export function Hero({ copy }: { copy: HeroContent }) {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[hsl(var(--brand-onyx))] text-[hsl(var(--brand-white))]">
      <div aria-hidden className="pointer-events-none absolute inset-0" style={{ background: 'radial-gradient(60% 72% at 80% 34%, hsl(47.22 95.58% 55.69% / 0.12) 0%, transparent 64%), radial-gradient(48% 60% at 8% 92%, hsl(0 0% 100% / 0.05) 0%, transparent 70%)' }} />
      ...
      <div className="relative mx-auto grid min-h-[30rem] max-w-7xl grid-cols-1 lg:min-h-[37rem] lg:grid-cols-[0.9fr_1.1fr]">
        <div className="loveli-reveal-up relative z-10 flex flex-col justify-center px-6 py-14 sm:py-18 lg:px-10 lg:py-20">
          ...
          <p className="mt-7 max-w-md text-base leading-8 text-[hsl(var(--brand-silver))] sm:text-lg">{copy.subhead}</p>
          <div className="mt-8 flex flex-wrap items-center gap-5">
            <Link href={copy.ctaHref} className="inline-flex items-center justify-center bg-[hsl(var(--brand-gold))] px-8 py-4 text-[10px] font-semibold uppercase tracking-[0.28em] text-[hsl(var(--brand-onyx))] transition duration-200 hover:-translate-y-0.5 hover:bg-[hsl(var(--brand-white))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--primary))] focus-visible:ring-offset-2">{copy.ctaLabel}</Link>
