...
For: the owner + any engineer on call · Written: 2026-05-30. **Updated
2026-06-03:** PayHero retired in favour of IntaSend (see
[docs/intasend-migration-2026-06.md](intasend-migration-2026-06.md)).
Sections below are rewritten for IntaSend; the archived PayHero runbooks
live at `docs/archive/`.

Production environment: Vercel + Supabase (project `thweaebhxsfxuxeosjty`,
"Loveli Luxury International", EU-West-1) + IntaSend + Resend + Sentry +
Upstash Redis.
...
| **Secrets** (Supabase service-role key, IntaSend secret + webhook challenge, Resend API key, Sentry auth token, Upstash token, Cron secret) | Vercel env vars (production scope) | NOT in any backup — rotate-on-loss policy | See §"Secrets recovery" |
| **IntaSend webhook registration** | IntaSend dashboard | NOT in our control | Re-register the webhook URL if it ever drops |
...
### 4. IntaSend webhook registration lost
...
**Recovery (≈ 5 minutes):**
1. IntaSend dashboard → Webhooks → set the URL to:
   ```
   https://loveli-luxury.vercel.app/api/intasend/webhook
   ```
2. Set the challenge value to the same string as `INTASEND_WEBHOOK_CHALLENGE`
   in Vercel env. The IntaSend webhook handler verifies every payload
   signature against this value; a mismatch returns 401 and the event is
   never applied.
...
3. If IntaSend secret: IntaSend dashboard → API Keys → Regenerate → update
   `INTASEND_SECRET_TOKEN` in Vercel + redeploy. If `INTASEND_WEBHOOK_CHALLENGE`
   was the leaked value, rotate it AND re-register the new value in the
   IntaSend dashboard's webhook settings.
...
| `INTASEND_PUBLISHABLE_KEY` | IntaSend dashboard → API Keys → publishable key |
| `INTASEND_SECRET_TOKEN` | IntaSend dashboard → API Keys → Regenerate secret |
| `INTASEND_WALLET_ID` | IntaSend dashboard → Wallets — copy the id of the float wallet |
| `INTASEND_WEBHOOK_CHALLENGE` | Generated by you (`openssl rand -hex 32`) — re-set in Vercel env + re-register in IntaSend webhook settings |
...
- `docs/intasend-migration-2026-06.md` — PayHero → IntaSend cutover plan
  and current phase status. The archived PayHero runbooks live at
  `docs/archive/`.
